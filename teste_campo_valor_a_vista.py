...
# Buscar uma comissão
result = supabase.table('comissoes_sienge_comissoes').select('id,sienge_id,numero_contrato').limit(1).execute()
...
if result.data:
    ...
    try:
        update_result = supabase.table('comissoes_sienge_comissoes').update({
        ...
        check = supabase.table('comissoes_sienge_comissoes').select('valor_a_vista').eq('id', c['id']).execute()
